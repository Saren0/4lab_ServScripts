...
public class Task4Helper {
	
	private final static String IllegalTypeError = "Неопределённый тип функции: ";

	public double calculateInDegrees(double number, Task4FunctionType ftype, 
			Integer accuracy) {
		double result;
		switch (ftype) {
			case COS:
	...
	public double calculateInRadians(double number, Task4FunctionType ftype, 
			Integer accuracy) {
		double result;
		switch (ftype) {
			case COS:
